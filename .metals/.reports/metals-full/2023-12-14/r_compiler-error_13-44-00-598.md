  ...
  def reconstruirCadenaTurbo(n: Int, o: Oraculo): Seq[Char] = {
    def generarSubC(k: Int, subCadena: Seq[Set[Seq[Char]]]): Seq[Set[Seq[Char]]] = {
      if (k > n) subCadena
        ...
        val nSubC = subCadena((k/2) - 1).flatMap(s1 => alfabeto.map(s2 => s1 ++ Seq(s2))).filter(o)
        generarSubC(k * 2, subCadena :+ nSubC)
     }
    }
    val subCadena = generarSubC(2, Seq(Set(Seq())))
    subCadena(n/2).find(_.length == n).getOrElse(Seq())
  }
}
